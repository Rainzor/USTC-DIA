...
def affine_backward(dout, cache):
    ...
    # dout.shape = (N, M), w.T.shape = (M, D)
    dx=np.dot(dout,w.T)
    
    # x.reshape(x.shape[0], -1).T.shape = (D, N)
    dw=np.dot(x.reshape(x.shape[0],-1).T,dout)

    # dout.shape = (N, M), db.shape = (M,)
...
def relu_backward(dout, cache):
    ...
    #在反向传播过程中，对于每个输入元素𝑥𝑖，我们只需要在该元素大于0时将dout传递下去，其他情况下将dout设置为0
    dx=dout*(x>0)
...
def svm_loss(x, y):
    ...
    for i in range(N):
        ...
        for j in range(C):
            ...
            margin = x[i, j] - correct_class_score + 1
            if margin > 0:
                loss += margin
                dx[i, j] = 1  # The gradient for incorrect classes
                dx[i, y[i]] -= 1  # The gradient for the correct class
